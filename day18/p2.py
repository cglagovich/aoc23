...
Digment = namedtuple('Digment', ('coord', 'ends_block'))
Coord = namedtuple("Coordinate", ('r', 'c'))

def dig(start, direc, steps):
    def move(start, direc, steps):
        return {
            'U': Coord(start.r-steps, start.c),
            'D': Coord(start.r+steps, start.c),
            'R': Coord(start.r, start.c+steps),
            'L': Coord(start.r, start.c-steps)
        }[direc]

    cur = Coord(start.r, start.c)
    new_digs = move(cur, direc, steps)
    ends_block = direc in 'RL'
    new_digs = Digment(cur, ends_block)
    return new_digs
...
def main():
    ...
    for line in inp:
        ...
        steps = int(val[2:-2], base=16)
        direc = direc_map[int(val[-2])]
        print(f'{direc} {steps}')
